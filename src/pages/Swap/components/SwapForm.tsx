...
const SwapForm: React.FC<SwapFormProps> = ({ address }) => {
  ...
  const [selectedFromToken, setSelectedFromToken] = useState<TokenType | null>(
    null
  );
  const [selectedToToken, setSelectedToToken] = useState<TokenType | null>(
    null
  );
        ...
        <div className='mb-6'>
          <div className='flex justify-between items-center mb-1 mx-1'>
            ...
            <span className='text-xs'>
              ...
              {selectedFromToken && renderTokenLabel(selectedFromToken)}
            </span>
          </div>
              ...
              <button className='flex items-center'>
                {selectedFromToken ? (
                  <>
                    <ImageWithFallback
                      src={
                        selectedFromToken.assets &&
                        selectedFromToken.assets.svgUrl
                          ? selectedFromToken.assets.svgUrl
                          : DEFAULT_SVG_URL
                      }
        ...
        <div className='mb-6'>
          <div className='flex justify-between items-center mb-1 mx-1'>
            ...
            <span className='text-xs'>
              ...
              {selectedToToken && renderTokenLabel(selectedToToken)}
            </span>
          </div>
          <div className='flex items-center p-3 rounded-xl bg-gray-100'>
            <input
              ...
              {dropdownOpenTo && (
                <div className='absolute mt-2 w-64 bg-white border border-gray-200 rounded-md shadow-lg z-10'>
                  {tokens
                    .filter(
                      (token) =>
                        token.identifier !== selectedFromToken?.identifier
                    )
                    .map((token) => (
                      <button
                        key={token.identifier}
      ...
      <div className='mt-6'>
        ...
        <div className='border border-gray-200 rounded-xl p-4'>
          <div className='flex justify-between text-sm'>
            ...
            <span>
              1 {selectedFromToken?.ticker} ≈ {amountOut}{' '}
              {selectedToToken?.ticker}
            </span>
          </div>
          <div className='flex justify-between text-sm'>
          ...
          <div className='flex justify-between text-sm'>
            ...
            <span>
              {(parseFloat(amountOut) * (1 - slippage / 100)).toFixed(2)}{' '}
              {selectedToToken?.ticker}
            </span>
          </div>
          <div className='flex justify-between text-sm'>
